"""
Optimized Color Detector with 4 Block State Output + Serial Transmission
功能：
 - 检测红、绿、蓝、金、银、黑六种颜色
 - 识别画面中面积最大的4个红/绿/蓝色块
 - 将颜色状态转化为 [0,1,1,0]（红蓝=0，绿=1），写入 block_states.txt
 - 额外：通过串口发送 "0 1 1 0" 格式字符串
 - 实时可视化并显示性能信息
 - 保留所有按键操作：q退出、s截图、+/-缩放
"""
import cv2
...
import os
import serial
import serial.tools.list_ports  # 用于查找串口
...
class OptimizedColorDetector:
    def __init__(self):
        ...
        self.last_block_states = None  # 用于避免重复发送
        ...
        if ports:
            ...
            try:
                ...
                print(f"串口已打开: {port_name} @ 9600 8N1")
    ...
    def update_block_file(self, detected_areas):
        ...
        try:
            ...
            print(f"方块状态更新: {block_states}")
        except Exception as e:
            print(f"写入block文件错误: {e}")

        # 2. 额外：发送到串口（仅当变化时）
        if block_states != self.last_block_states and self.ser is not None and self.ser.is_open:
            msg = ' '.join(map(str, block_states)) + '\n'  # 格式如 "0 1 1 0\n"
            try:
                self.ser.write(msg.encode('utf-8'))
                print(f"串口发送: {msg.strip()}")
