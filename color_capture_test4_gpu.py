...
class HighPerformanceColorDetector:
    def __init__(self):
        ...
        # 性能优化设置
        self.processing_scale = 0.4  # 更低的处理比例以提高性能
    ...
    def optimized_color_detection(self, frame, hsv):
        """优化的颜色检测方法 - 不使用GPU"""
            ...
            for contour in contours:
                ...
                if area > min_area:
                    ...
                    if perimeter > 0:
                        ...
                        if circularity < 0.25:  # 降低阈值以检测更多形状
                            continue
    ...
    def detect_colors(self, frame):
        ...
        # 使用优化的颜色检测
        detected_areas = self.optimized_color_detection(process_frame, hsv)
    ...
    def add_ui_elements(self, frame, detected_count):
        ...
        # 添加标题
        title = "High-Performance Color Detection"
        ...
        # 添加处理模式信息
        mode_text = f"Processing Scale: {self.processing_scale}"
        mode_size = cv2.getTextSize(mode_text, cv2.FONT_HERSHEY_COMPLEX, 0.5, 1)[0]
        self.put_pretty_text(frame, mode_text, (w - mode_size[0] - 20, 120), 
                           (0, 255, 0), 0.5, 1)
