...
class OptimizedColorDetector:
    def __init__(self):
        self.color_ranges = {
            'Red': [
                ([0, 150, 100], [10, 255, 255]),      # 缩小红色范围，避免与金色重叠
                ([170, 150, 100], [180, 255, 255])    # 提高饱和度阈值
            ],
            'Green': [
                ([40, 80, 60], [80, 255, 255])        # 稍微扩大绿色范围
            ],
            'Blue': [
                ([100, 100, 80], [130, 255, 255])     # 调整蓝色范围
            ],
            'Gold': [
                ([20, 100, 120], [30, 200, 220])      # 缩小金色范围，避免与红色重叠
            ],
            'Silver': [
                ([0, 0, 150], [180, 30, 230])         # 调整亮度范围
            ],
            'Mineral_Gray': [
                ([0, 0, 50], [180, 30, 180])          # 更严格的矿物检测范围
            ]
        }
        ...
        self.ser = None
        self.lock_counter_threshold = 30
    ...
    def detect_mineral_above(self, hsv_full, x, y, w, h):
        # 更精确的矿物搜索区域
        roi_h = int(h * 0.8)  # 稍微扩大搜索高度
        roi_y_start = max(0, y - roi_h - 10)  # 扩大搜索起始位置
        roi_y_end = max(0, y - 5)
        ...
        # 应用高斯模糊减少噪声
        roi_blur = cv2.GaussianBlur(roi_hsv, (3, 3), 0)
        
        ranges = self.color_ranges['Mineral_Gray']
        mask = None
        for lower, upper in ranges:
            temp_mask = cv2.inRange(roi_blur, np.array(lower), np.array(upper))
            if mask is None:
                mask = temp_mask
            else:
        ...
        if mask is not None:
            ...
            # 调整检测阈值
            if count > (total_pixels * 0.15) and count > 80:  # 提高阈值
                # 计算矿物质心
                M = cv2.moments(mask)
                if M["m00"] != 0:
    ...
    def update_block_file(self, detected_areas):
        ...
        # 添加面积过滤，避免小噪声被识别
        min_valid_area = 800  # 根据实际情况调整
        top4 = [a for a in top4 if a['size'] > min_valid_area]
    ...
    def process_frame(self, frame):
        ...
        if self.is_locked:
             ...
             while len(current_minerals) < 2: 
                 current_minerals.append(0)
