...
"""
todo:
1.清洗数据: 只有颜色状态变化的时候才发送
2.灰度调整: 只有在蓝色或红色的矿物上的黑灰色才识别为2
3.六元数组的创建: 前四位颜色板载识别确定后就不再改变 之后只发送两位数组表示：第几个红色/蓝色板上方有矿物
4.实现可使用的串口检测: 从COM10到COM19任意串口发送
5.串口发送的数据同步记录在block_states.txt文件中
...
class OptimizedColorDetector:
    def __init__(self):
        ...
        try:
            ...
            print("sending test: hello_serial")
    ...
    def update_block_file(self, detected_areas):
        ...
        self.warning_flag = False   # 默认无警告

        for i, area in enumerate(top4):
            if area['name'] in ['Red', 'Blue']:
                bx, by = area['center']
                ...
                for silver in silvers:
                    ...
                    # 条件：银色矿在红/蓝板上方 & 水平对齐
                    if sy < by and abs(sx - bx) < area['size'] * 1.2:
                        self.warning_flag = True
                        print("******WARNING: Silver detected******")
                        break
        try:
    ...
    def process_frame(self, frame):
        ...
        for area in detected_areas:
            ...
            cv2.putText(result, area['name'], area['center'],
                ...
                # ============================================================
        # 水平线检测 —— 获取所有主色板(R/G/B)的平均 Y 坐标
        # ============================================================
        main_blocks = [a for a in detected_areas if a['name'] in ['Red', 'Green', 'Blue']]
        if len(main_blocks) > 0:
            avg_y = int(sum([a['center'][1] for a in main_blocks]) / len(main_blocks))
            h, w = result.shape[:2]
            # 绘制水平虚线（绿色）
            for x in range(0, w, 30):
                cv2.line(result, (x, avg_y), (x + 15, avg_y), (0, 255, 0), 2)

        # ============================================================
        # 若检测到红/蓝上方的银色矿物，则警告显示
        # ============================================================
        if hasattr(self, "warning_flag") and self.warning_flag:
            cv2.putText(result,
                        "******WARNING: Silver above RED/BLUE plate!******",
                        (50, 150),
