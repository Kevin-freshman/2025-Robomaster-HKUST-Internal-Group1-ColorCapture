...
class OptimizedColorDetector:
    ...
    def check_color_proximity(self, color_areas):
        """检查红色或蓝色是否在绿色边上"""
        green_areas = [area for area in color_areas if area['name'] == 'Green']
        red_areas = [area for area in color_areas if area['name'] == 'Red']
        blue_areas = [area for area in color_areas if area['name'] == 'Blue']
        ...
        for red_area in red_areas:
            for green_area in green_areas:
                if self.is_nearby(red_area, green_area):
                    return True
        
        # 检查每个蓝色区域是否靠近绿色区域
        for blue_area in blue_areas:
            for green_area in green_areas:
                if self.is_nearby(blue_area, green_area):
                    return True
                    
        return False
    
    def is_nearby(self, area1, area2, distance_threshold=150):
        """判断两个颜色区域是否相邻"""
        x1, y1 = area1['center']
        x2, y2 = area2['center']
        ...
        # 考虑区域大小，计算一个动态阈值
        size_factor = (area1['size'] + area2['size']) / 2
        dynamic_threshold = distance_threshold + size_factor * 0.5
    ...
    def fast_color_detection(self, process_frame, original_dims):
        ...
        # 检查颜色相邻状态并更新文件
        is_near_green = self.check_color_proximity(detected_areas)
        self.update_status_file(1 if is_near_green else 0)
    ...
    def add_performance_info(self, frame, detected_count):
        ...
        # 显示当前状态
        current_status = self.last_status if self.last_status is not None else 0
        status_color = (0, 255, 0) if current_status == 1 else (200, 200, 200)
        status_text = f"Detected: {detected_count} colors | Status: {current_status} | Q:Quit | S:Save | +/-:Scale"
...
def main():
    ...
    print(f"状态文件: {detector.status_file}")
