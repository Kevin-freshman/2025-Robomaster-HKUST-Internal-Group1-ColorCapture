...
class OptimizedColorDetector:
    def __init__(self):
        ...
        try:
            # !! 非常重要 !!
            # 请将 'COM3' 修改为您设备实际连接的串口号 (例如 'COM3', 'COM4'...)
            # 或 Linux/Mac 上的 '/dev/ttyUSB0', '/dev/ttyACM0' 等
            serial_port = 'COM3'  
            baud_rate = 9600  # 波特率应与您设备（如Arduino）设置的波特率一致
    ...
    def update_block_file(self, detected_areas):
        ...
        try:
            ...
            if self.ser and self.ser.is_open:
                # 2. 将列表转换为字符串，并添加换行符 (例如: "[-1, 0, 1, -1]\n")
                # 换行符有助于接收端（如Arduino）解析数据
                data_to_send = f"{str(block_states)}\n"
                
                # 3. 编码为字节并发送
                self.ser.write(data_to_send.encode('utf-8'))
                # print(f"已发送到串口: {data_to_send.strip()}") # 取消此行注释以进行调试
            # --- 新增结束 ---
