"""
Optimized Color Detector with 4 Block State Output to Serial
功能：
 - 检测红、绿、蓝、金、银、黑六种颜色
 - 识别画面中面积最大的4个红/绿/蓝色块
 - 将颜色状态转化为 [0,1,1,0]（红蓝=0，绿=1），发送到串口（格式"0 1 1 0"）
 - 实时可视化并显示性能信息
"""
import cv2
import numpy as np
...
import serial.tools.list_ports  # 用于查找可用端口
...
class OptimizedColorDetector:
    def __init__(self):
        ...
        self.last_block_states = None  # 新增：跟踪上次block_states，避免重复发送

        # 串口初始化：查找第一个可用COM端口，打开9600波特率
        ports = serial.tools.list_ports.comports()
        if not ports:
            print("未检测到任何COM端口！")
            self.ser = None
        else:
            port_name = ports[0].device  # 取第一个端口
            try:
                self.ser = serial.Serial(port_name, 9600, timeout=1)
                print(f"串口 {port_name} 已打开 (9600 baud)")
            except Exception as e:
                print(f"打开串口错误: {e}")
                self.ser = None

        print("优化版高性能颜色识别系统初始化完成")
        print("目标颜色: 红, 绿, 蓝, 金, 银, 黑")
    ...
    # 修改：发送block_states到串口（如果变化），格式"0 1 1 0"
    def update_block_file(self, detected_areas):
        ...
        if block_states != self.last_block_states and self.ser is not None:
            msg = ' '.join(map(str, block_states))  # 转换为"0 1 1 0"
            try:
                self.ser.write(msg.encode())  # 发送字节
                print(f"方块状态更新并发送: {block_states} (msg: {msg})")
                self.last_block_states = block_states
            except Exception as e:
                print(f"串口发送错误: {e}")
...
def main():
    ...
    try:
        ...
        if hasattr(detector, 'ser') and detector.ser:
            detector.ser.close()  # 关闭串口
